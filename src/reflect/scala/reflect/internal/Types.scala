...
//      maybeRewrap(underlying.instantiateTypeParams(formals, actuals))
    ...
    override def skolemizeExistential(owner: Symbol, origin: AnyRef) = {
      ...
      def newSharpenedSkolem(basis: Symbol, tparam: Symbol): Symbol = {
        ...
        val canSharpen = ( emptyBounds(quant) && !emptyBounds(tparam)
          && (existentialsInType(tparam.info) intersect quantified).isEmpty )

        val basis =
          if (canSharpen) quant.cloneSymbol(quant.owner) setInfo tparam.info.substSym(tpars, quantified)
          else quant

        newSkolem(basis) // TODO: does this properly skolemize the `quantified` that may appear in `basis.info`?
      }
