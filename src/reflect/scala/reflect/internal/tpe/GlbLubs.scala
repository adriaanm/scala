...
private[internal] trait GlbLubs {
  ...
  private final val experimentalLubs = scala.sys.props contains "scalac.experimental.lub"
  ...
  // only called when experimentalLubs
      ...
      if (tsBts.isEmpty || (tsBts exists typeListIsEmpty)) pretypes.reverse
        ...
        if (isUniformFrontier) {
          ...
          mergePrefixAndArgs(ts1, Covariant, depth) match {
            ...
            case tp if experimentalLubs && willViolateRecursiveBounds(tp, ts0, ts1) =>
