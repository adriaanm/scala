...
import scala.reflect.internal.interactive.Completion.{Candidates, ScalaCompleter}


class PresentationCompilerCompletion(intp: IMain) extends Completion {
  private case class Request(line: String, cursor: Int)
  private def initialRequest = new Request("", -1)
  private var lastRequest = initialRequest
  private var tabCount = 0

  def resetVerbosity(): Unit = { tabCount = 0; lastRequest = initialRequest }

  def completer(): ScalaCompleter = new ScalaCompleter {
