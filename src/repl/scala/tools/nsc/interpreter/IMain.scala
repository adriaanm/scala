  ...
  */
  ...
  private[interpreter] def requestFromLine(input: String, synthetic: Boolean = false): Either[Result, Request] =
    parse(input) flatMap {
      case Nil => Left(Error)
      case trees =>
        executingRequest = new Request(input, trees, synthetic = synthetic)
  ...
  class Request(val line: String, origTrees: List[Tree], generousImports: Boolean = false, synthetic: Boolean = false) extends ReplRequest {
    ...
    private def storeInVal(tree: Tree): Tree = {
      ...
      atSynthPos(ValDef(NoMods, resName, TypeTree(), tree))
    }
    ...
    private object spliceUserCode extends Transformer {
      override def transformStats(stats: List[Tree], exprOwner: Symbol): List[Tree] =
        stats flatMap {
          case Ident(USER_CODE_PLACEHOLDER) => trees
          case t                    => List(super.transform(t))
        }
    }

    private def parseSynthetic(code: String): List[Tree] = newUnitParser(code, "<synthetic>").parseStats()
    ...
    def mkUnit: CompilationUnit = {
      ...
      // override positions on the path from `unit.body` to the user code (`trees` -- spliced in next),
      // so that pres compiler can locate the user input
...
//        println(unit.body)
  ...
  def parse(line: String): Either[Result, List[Tree]] = {
