...
trait MatchTranslation {
  ...
  trait MatchTranslator extends TreeMakers with TreeMakerWarnings {
    ...
    def newBoundTree(tree: Tree, pt: Type): BoundTree = tree match {
      case SymbolBound(sym, expr) => BoundTree(SymbolScrutinee(setVarInfo(sym, pt)), expr)
      case _                      => BoundTree(SymbolScrutinee(setVarInfo(freshSym(tree.pos, prefix = "p"), pt)), tree)
    }
