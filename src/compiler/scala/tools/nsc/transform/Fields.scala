...
import scala.reflect.internal.Flags.{ACCESSOR => _, DEFERRED => _, FINAL => _, IMPLICIT => _, LAZY => _, METHOD => _, MIXEDIN_ACCESSOR => _, MODULE => _, MODULEVAR => _, MUTABLE => _, NEEDS_TREES => _, OVERRIDE => _, PRESUPER => _, PRIVATE => _, PROTECTED => _, PrivateLocal => _, ProtectedLocal => _, STABLE => _, SYNTHESIZE_IMPL_IN_SUBCLASS => _, SYNTHETIC => _, TRANS_FLAG => _, lateDEFERRED => _, notPROTECTED => _, _}
...
abstract class Fields extends InfoTransform with ast.TreeDSL with TypingTransformers {
  ...
  private def moduleInit(moduleVar: Symbol, module: Symbol) =
  ...
  def newModuleVarSymbol(site: Symbol, accessor: Symbol, tp: Type, extraFlags: Long): TermSymbol = {
    println(s"new module var in $site for $accessor of type $tp")
    site.newVariable(nme.moduleVarName(accessor.name.toTermName), accessor.pos.focus, MODULEVAR | extraFlags) setInfo tp addAnnotation VolatileAttr
  }
  ...
  // reuse symbol since it can be referenced from anywhere TODO: change scheme to always use accessor
  def convertModuleToAccessor(module: Symbol, moduleVar: Symbol = NoSymbol): Unit = {
    module setFlag METHOD | STABLE // keep original info, don't convert to method type
    if (moduleVar.exists) moduleVarOf(module) = moduleVar
  }
  ...
  private object synthFieldsAndAccessors extends TypeMap {
    ...
    def newModuleAccessor(module: Symbol, site: Symbol, moduleVar: Symbol) = {
      ...
      // for use when we're in the same prefix as module, so no need for site.thisType.memberType(module)
      ...
      case tp@ClassInfoType(parents, decls, clazz) if clazz.isTrait =>
        ...
        origDecls.foreach { member =>
          if (member hasFlag ACCESSOR) {
            ...
            convertModuleToAccessor(member)
          ...
          if (member hasFlag MODULE) {
            if (member.isStatic) { // expanding module def (top-level or nested in static module)
              ...
              if (member hasFlag SYNTHESIZE_IMPL_IN_SUBCLASS) List(moduleVar, newModuleAccessor(member, clazz, moduleVar))
                ...
                convertModuleToAccessor(member, moduleVar)
                member setFlag NEEDS_TREES
                List(moduleVar)
              }
            }
    ...
    def fieldsAndAccessors(exprOwner: Symbol): List[ValOrDefDef] = {
      if (exprOwner.isLocalDummy) {
        ...
        def moduleAccessorBody(module: Symbol): Some[Tree] = Some(
          ...
          else moduleInit(moduleVarOf(module), module)
        )
    ...
    private def transformStat(exprOwner: Symbol)(stat: Tree): List[Tree] = {
      ...
      stat match {
        ...
        case ModuleDef(_, _, impl) =>
          ...
          val cd = ClassDef(stat.symbol.moduleClass, super.transformTemplate(impl)) setType NoType
          if (clazz.isClass) List(cd)
            ...
            convertModuleToAccessor(module) // mutates module.info
            cd :: mkField(moduleVar) :: mkAccessor(module)(moduleInit(moduleVar, module)) :: Nil
          }
