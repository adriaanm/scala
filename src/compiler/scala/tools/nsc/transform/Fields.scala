...
abstract class Fields extends InfoTransform with ast.TreeDSL with TypingTransformers {
  ...
  def convertModuleToAccessor(module: Symbol, moduleVar: Symbol = NoSymbol): Unit = {
    module setFlag METHOD | STABLE
    module setInfo MethodType(Nil, module.info.resultType) // we're in the same prefix as module, so no need for site.thisType.memberType(module)
  ...
  private object synthFieldsAndAccessors extends TypeMap {
    ...
    def newMatchingModuleAccessor(clazz: Symbol, member: Symbol): MethodSymbol =
      clazz.newMethod(member.name.toTermName, member.pos, (member.flags | NEEDS_TREES | STABLE) & ~MODULE) setInfo MethodType(Nil, member.moduleClass.tpe)
        ...
        val synthAccessorAndFields = membersNeedingSynthesis map { member =>
          ...
          if (member hasFlag MODULE) {
            if (member.isStatic) { // expanding module def (top-level or nested in static module)
              ...
              val moduleVar = clazz newModuleVarSymbol member setInfo site.memberType(member).resultType setFlag NEEDS_TREES
    ...
    def fieldsAndAccessors(exprOwner: Symbol): List[ValOrDefDef] = {
      if (exprOwner.isLocalDummy) {
        ...
        def moduleAccessorBody(module: Symbol): Some[Tree] = Some(
          ...
          else if (module.isStatic) gen.mkAttributedRef(clazz.thisType, module)
      ...
      stat match {
        ...
        case ModuleDef(_, _, impl) =>
          ...
          if (clazz.isClass) List(cd)
            ...
            val moduleVar = currentOwner newModuleVarSymbol module setInfo module.info
