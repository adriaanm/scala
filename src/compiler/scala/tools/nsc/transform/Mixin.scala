...
//      println(s"starting rebindsuper $base mixing in from $mixinClass: $member:${member.tpe} of ${member.owner} ; looking for super in $bcs (all bases: ${base.info.baseClasses})")
      while (!bcs.isEmpty && sym == NoSymbol) {
        if (settings.debug) {
          ...
          debuglog(s"rebindsuper ${bcs.head} $other ${other.tpe} ${other.isDeferred}")
        }
        // don't rebind to specialized members since we can't jump back and forth between the unspecialized name and specialized one
        // (So we jump into the non-specialized world and stay there until we hit our super.)
        sym = member.matchingSymbol(bcs.head, base.thisType).suchThat(sym => !sym.hasFlag(DEFERRED | BRIDGE | SPECIALIZED))
        bcs = bcs.tail
      }
