  ...
  class ExplicitOuterTransformer(unit: CompilationUnit) extends OuterPathTransformer(unit) {
    ...
    override def transform(tree: Tree): Tree = {
      ...
      tree match {
        ...
        case treeInfo.Applied(sel @ Select(qual, name), _, List(args)) if (name == nme.CONSTRUCTOR || sel.symbol.isStaticMember) && isInner(sel.symbol.owner) && !sel.symbol.owner.isTrait =>
