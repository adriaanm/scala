...
trait Contexts { self: Analyzer =>
  ...
  class Context private[typechecker](val tree: Tree, val owner: Symbol, val scope: Scope,
    ...
    lazy val depth: Int = {
      val increasesDepth = isRootImport || outerIsNoContext || (outer.scope != scope) || treeInfo.magicImportBumpsNesting(tree)
    ...
    /** A root import is never unused and always bumps context depth. (scala/Predef/java.lang) */
  ...
  trait ImportContext extends Context {
    // I didn't investigate, but, unless this is a val, we miss unused imports in macro expansions (test/files/neg/warn-unused-imports)
    private[this] val impInfo: ImportInfo = {
      val info = new ImportInfo(tree.asInstanceOf[Import], outerDepth)
      if (settings.warnUnusedImport && !(isRootImport || treeInfo.magicImportBumpsNesting(tree)))
