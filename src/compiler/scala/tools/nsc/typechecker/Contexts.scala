...
trait Contexts { self: Analyzer =>
  ...
  class Context private[typechecker](val tree: Tree, val owner: Symbol, val scope: Scope,
    ...
    def deprecationWarning(pos: Position, sym: Symbol, msg: String, since: String): Unit = currentRun.reporting.deprecationWarning(fixPosition(pos), sym, msg, since, owner.fullName)
    def deprecationWarning(pos: Position, sym: Symbol): Unit = currentRun.reporting.deprecationWarning(fixPosition(pos), sym, owner.fullName)
