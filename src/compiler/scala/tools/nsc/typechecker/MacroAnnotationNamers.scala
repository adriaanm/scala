...
trait MacroAnnotationNamers { self: Analyzer =>
  ...
  private class MacroAnnotationNamer(context: Context) extends Namer(context) {
    ...
    private def createAssignAndEnterSymbol(tree: Tree, mask: Long = -1L): Symbol = {
    ...
    /** Does the same as `ensureCompanionObject`, but also makes sure that the returned symbol destroys itself
      ...
      */
    // TODO: deduplicate
    private def weakEnsureCompanionObject(cdef: ClassDef, creator: ClassDef => Tree = companionModuleDef(_)): Symbol = {
      val m = patchedCompanionSymbolOf(cdef.symbol, context)
      if (m != NoSymbol && currentRun.compiles(m)) m
      else { val mdef = atPos(cdef.pos.focus)(creator(cdef)); enterSym(mdef); markWeak(mdef.symbol) }
    }

    private def finishSymbol(tree: Tree): Unit = {
    ...
    private def finishSymbolNotExpandee(tree: Tree): Unit = {
      val sym = tree.symbol
    ...
    private abstract class MaybeExpandeeCompanionCompleter(tree: Tree) extends MaybeExpandeeCompleter(tree)
    ...
    private def finishSymbolMaybeExpandee(tree: Tree, annZippers: List[treeInfo.AnnotationZipper]) {
      val sym = tree.symbol
      unmarkWeak(sym)
      ...
      sym.setInfo(new MaybeExpandeeCompleter(tree) {
        ...
        override def maybeExpand(): Unit = {
          ...
          def maybeExpand(annotation: Tree, annottee: Tree, maybeExpandee: Tree): Option[List[Tree]] = {
            ...
            if (mann.isClass && mann.asClass.isMacroAnnotation && context.macrosEnabled) {
    ...
    private def finishSymbolMaybeExpandeeCompanion(tree: Tree, m: Symbol, c: Symbol) {
    ...
    private def probeMacroAnnotation(context: Context, tpt: Tree): Symbol = {
    ...
    private def patchedCompanionSymbolOf(original: Symbol, ctx: Context): Symbol = if (original == NoSymbol) NoSymbol else {
    ...
    private def prepareAnnotationMacro(ann: Tree, mann: Symbol, sym: Symbol, annottee: Tree, expandee: Tree): Tree = {
    ...
    private def expandAnnotationMacro(original: Tree, expandee: Tree): Option[List[Tree]] = {
