...
//      println(s"qualifying class in ${context} / ${System.identityHashCode(context)} / ${System.identityHashCode(context.outer)}")
      ...
      templ.parents match {
        ...
        case parents =>
          try {
             ...
             *
             * TODO: what if a (non-trait) class actually appears later in the list?
             */
            def normalizeFirstParent(parents: List[Tree]): List[Tree] = {
              @annotation.tailrec
              def explode0(parents: List[Tree]): List[Tree] = {
                ...
                if (supertpt.tpe.typeSymbol == AnyClass) {
                  ...
                  // TODO when would supertpt not be typed already?? it's either coming from the original list of typed parents or its the TypeTree created below
                  val supertpt1 = typedParentType(supertpt)

                  if (supertpt1.isErrorTyped) rest
                  else explode0((TypeTree(supertpt1.tpe.firstParent) setPos supertpt.pos.focus) :: supertpt1 :: rest)
                } else parents
              }
             ...
             */
            def fixDuplicateSyntheticParents(parents: List[Tree]): List[Tree] = parents match {
              case Nil     => Nil
              case parent :: xs =>
                val parentSym = parent.tpe.typeSymbol
                parent :: fixDuplicateSyntheticParents(
                  if (isPossibleSyntheticParent(parentSym)) xs filterNot (_.tpe.typeSymbol == parentSym)
                  else xs
                )
            }
    ...
    def typedClassDef(cdef: ClassDef): Tree = {
      ...
      try {
        ...
        val templateTyper = newTyper(context.make(cdef.impl, clazz, newScope))
        val impl1    = templateTyper.typedTemplate(cdef.impl, this)
    ...
    def typedModuleDef(mdef: ModuleDef): Tree = {
      ...
      try {
        ...
        val impl0 =
          if (noSerializable) mdef.impl
          else {
            clazz.makeSerializable()
            treeCopy.Template(mdef.impl, mdef.impl.parents ++ (TypeTree(SerializableTpe).setPos(clazz.pos.focus) :: Nil), mdef.impl.self, mdef.impl.body)
          }

        val templateTyper = newTyper(context.make(mdef.impl, clazz, newScope))
        val impl1 = templateTyper.typedTemplate(impl0, this)
        val impl2 = finishMethodSynthesis(impl1, clazz, context)
    ...
    def typedTemplate(templ0: Template, clazzTyper: Typer): Template = {
      val templ = templ0
...
//        println(s"updating parents of $clazz from ${clazz.info.parents} to $parentTypes")
      ...
      val templContext = context.outer.make(templ, clazz, clazz.info.decls)
      enterSyms(templContext, body1)

      // if `parentTypes` has invalidated the template, don't validate it anymore
      if (!templ.isErrorTyped)
      ...
      if (clazz.isTrait)
        ...
        // TODO: the first, and only the first, class to mix in a trait with parameters must pass arguments for them
      }
          ...
          def tryNamesDefaults: Tree = {
            ...
            if (mt.isErroneous) duplErrTree
              ...
              if (fun1.isErroneous) duplErrTree
                ...
                val NamedApplyBlock(NamedApplyInfo(staticQual, qual, targs, previousArgss, _)) = fun1
                ...
                val (allArgs, missing) = addDefaults(args, staticQual, qual, targs, previousArgss, params, fun.pos.focus, context)
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
      ...
      def typedNew(tree: New) = {
        ...
        if (isPrimitiveValueClass(sym)) {
          ...
          setError(tpt)
        } else treeCopy.New(tree, tpt1).setType(tp)
      }
