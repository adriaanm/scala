...
trait ScalaSettings extends AbsScalaSettings
  ...
  val deprecationSuppress = MultiStringSetting("-deprecation-suppress", "conditions", "Conditions for suppressing deprecation warnings, `help` for details.",
                              helpText = Some("""Specify when to suppress deprecation warnings, based on the "since" argument
                                | of the annotation, or the referencing or defining package/compilation unit.
                                |
                                | Conditions are or-ed together after partitioning by being prefixed by '!'.
                                | A '!' prefix escalates the warning to an error, unless there is a matching
                                | condition that suppresses it.
                                |
                                |  after <version> // "since" argument of `@deprecated` compares >= to <version>
                                |  in <pkg>        // fully qualified name of deprecated member starts with <pkg>
                                |  from <pkg>      // deprecation warning generated by unit under <pkg>
                                |
                                |<version> is parsed and compared as a Scala version number;
                                |<pkg> is a prefix of a fully qualified name.
                                |
                                |The setting accepts a list of conditions: `-deprecation-suppress:c1:c2`.
                                |The setting can be passed multiple times, extending the list of conditions.""".stripMargin))
