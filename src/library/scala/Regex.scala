...
class Regex private[scala](val pattern: Pattern, groupNames: String*) extends Serializable {
  outer =>
...
object Regex {
  ...
  class Match(val source: CharSequence,
              protected[scala] val matcher: Matcher,
  ...
  class MatchIterator(val source: CharSequence, val regex: Regex, val groupNames: Seq[String])
    ...
    private[scala] def replacementData = new AbstractIterator[Match] with Replacement {
  ...
  /**
    ...
    */
  private[scala] trait Replacement {
    protected def matcher: Matcher
